...
- **A typeahead in the text editor appears where you are typing, inside an overlay too.** The `@`-mention list and your own typeaheads in `nldd-text-editor` worked, but you could not see them once the editor sat in an `nldd-sheet`, an `nldd-modal-dialog` or an `nldd-window`: the list landed beside the page. CodeMirror measures that list against the viewport, and an ancestor with a transform takes over as the frame it is measured in, so the overlay's offset was counted twice. Two things changed. The list is measured against the editor now, so it follows the editor wherever that sits, including a container of your own that carries a transform, and it stays inside the box that would cut it off: an overlay or a container that hides its overflow now shifts the list to fit instead of clipping it. And the overlays keep no transform once they are open: the animation that slides an `nldd-sheet` or an `nldd-modal-dialog` in used to leave the position it ended on behind. Anything else in an overlay that positions itself against the viewport, a popover or a menu of your own, lands right for that same reason.
