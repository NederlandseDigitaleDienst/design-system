...
[zoekveld](?path=/docs/components-inputs-search-field--docs) boven de lijst
...
[token](?path=/docs/components-content-token--docs) per actieve waarde zegt
...
[container](?path=/docs/components-layout-container--docs) is `stack`, en dan
...
[sheet](?path=/docs/components-layout-sheet--docs) is verborgen tot je `open`
...
staat in [bewerken in een sheet](?path=/docs/patronen-bewerken-in-een-sheet--docs).
...
zie [lijst met rijen](?path=/docs/patronen-lijst-met-rijen--docs).
