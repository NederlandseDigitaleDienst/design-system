...
**Wanneer niet.** Voor een korte bevestiging is een sheet te zwaar, zie [bevestigen](?path=/docs/patronen-bevestigen--docs). Voor een licht paneel dat aan één knop hangt, zoals een snelle filter, gebruik je een [popover](?path=/docs/components-layout-popover--docs). En bekijken en bewerken zijn verschillende taken: pers ze niet in één scherm met inline bewerken.
...
**De uitweg staat bovenin, de primaire actie onderin.** "Sluiten" zit in de [titelbalk](?path=/docs/components-navigation-top-title-bar--docs) en "Bewaar" in de footer. Zo staat de uitweg niet naast de knop waar de gebruiker op de automatische piloot naartoe gaat. Zet dus geen "Annuleer" naast "Bewaar"; zie de [ontwerprichtlijnen](?path=/docs/docs-ontwerprichtlijnen--docs).

**Open de sheet met `open`, en laat hem in de DOM staan.** De [sheet](?path=/docs/components-layout-sheet--docs) zet `open` zelf weer uit als de gebruiker hem sluit, met Esc, een klik ernaast of de sluitknop. Bind `open` daarom samen met `close` aan je eigen toestand:
...
Mount je de sheet pas op het moment dat hij open moet, dan slaat de animatie over en verlies je wat er in het formulier stond. Het complete Vue-component staat in [bootstrap-vue](https://github.com/NederlandseDigitaleDienst/design-system/blob/main/skills/nldd-design-build/examples/bootstrap-vue.md).
