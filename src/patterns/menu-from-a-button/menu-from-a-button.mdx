...
**Wanneer niet.** Voor de hoofdnavigatie van een site gebruik je een [menu bar](?path=/docs/components-navigation-menu-bar--docs). Voor een keuze uit veel opties in een formulier gebruik je een [dropdown](?path=/docs/components-inputs-dropdown--docs). En geen megamenu's, zie de [ontwerprichtlijnen](?path=/docs/docs-ontwerprichtlijnen--docs).
...
**Nest het menu in de `popup`-slot van de knop.** Dan hangt de [knop](?path=/docs/components-actions-button--docs) het menu zelf aan zich vast en opent en sluit hij het: geen id, geen `anchor`, geen eigen klikafhandeling. Het menu meldt `expanded` en `aria-haspopup` terug aan de knop, vanaf de eerste render. Een losse `anchor` is alleen nodig voor een trigger zonder `popup`-slot.

**Een destructieve actie staat onderaan, achter een scheidingslijn.** Zo staat hij niet tussen de acties waar de gebruiker snel doorheen klikt. `destructive` kleurt het [item](?path=/docs/components-actions-menu--docs) rood, maar de tekst moet zelf zeggen wat er gebeurt. Is de actie onomkeerbaar, vraag dan om [bevestiging](?path=/docs/patronen-bevestigen--docs) of maak hem ongedaan te maken.
...
**Een actie in een werkbalk hoort ook in het overloopmenu.** Hoe dat werkt staat in [werkbalk met acties](?path=/docs/patronen-werkbalk-met-acties--docs).
